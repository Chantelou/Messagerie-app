...
function AudioSend({ setRecording, recording, handleSendMessage }: { 
    ...
    const formatTime = (time: number) => {
        ...
        return `${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}` // Retourne le temps formaté en mm:ss
    }
    ...
    const SendAudioToImageKit = async () => {
        ...
        if (!audioBlobToSend) return ToastError("Aucun fichier audio sélectionné");
        if (audioBlobToSend.size > 10 * 1024 * 1024) return ToastError("Fichier audio trop volumineux (>10MB)");

        // Convertir le Blob en File (ImageKit préfère File, mais Blob fonctionne aussi)
        const audioFile = new File([audioBlobToSend], `${Date.now()}.webm`, { type: 'audio/webm' });
                ...
                <div className='flex w-full justify-between items-center'>
                    ...
                    {audioBase64 && audioBase64 !== "" ? (
                        <div className='flex justify-end flex-col w-full'>
                            <audio src={audioBase64 || ""} controls className='w-full mb-3'></audio>
                            {!loader ? (
                                <div className="flex items-center justify-end gap-5">
                                <button onClick={() => setRecording(false)} type='button' className="btn rounded-3xl bg-red-500 hover:bg-red-400 text-white ">Annuler</button>
                                <button type='button' onClick={() => SendAudioToImageKit()} className="btn rounded-3xl bg-green-500 hover:bg-green-400 text-white ">Envoyer</button>
                            </div>
                            ): (
                                <button type='button' className="btn rounded-3xl bg-green-500 hover:bg-green-400 text-white " disabled>Envoi...</button>
                            )}
                        </div>
                    ) : (
                        <>
                            ...
                            <button onClick={stopRecording} className="send-button bg-red-500 hover:bg-red-400"><StopCircle /></button>
                        </>
                    )}

                </div>
            ) : (
                <button onClick={startRecording} className="send-button bg-red-500 hover:bg-red-400"><Mic size={20} /></button>
            )}
        </>
    )
}
